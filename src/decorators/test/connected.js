...
export function connected(path, options) {
    ...
    return function(target, name, descriptor) {
        ...
        descriptor.get.getObserver = function getObserver(obj) {
           if (!this._au_redux_observers) {
               this._au_redux_observers = {};
           }

           if (!this._au_redux_observers[name]) {
               this._au_redux_observers[name] = new ConnectedObserver(name, obj, path, options);
           }

           return this._au_redux_observers[name];
       };
        ...
        let subscription;
         ...
         target.bind = function bind(...rest) {
             subscription = subscriptionFactory(this, name);
...
function subscriptionFactory(context, name) {
    return function (newValue, oldValue) {
        if (context[name + 'Changed']) {
            context[name + 'Changed'](newValue, oldValue);
        }
    }
}
